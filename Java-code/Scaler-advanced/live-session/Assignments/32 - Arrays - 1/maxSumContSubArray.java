...
public class maxSumContSubArray {
    public int maxSubArray_O_N2_(final List<Integer> A) {
        int N = A.size();
    ...
    public int maxSubArray(final List<Integer> A) {
        ...
        // Since the List is declared as final, created an separate array with the same value.
        int[] B = new int[N];
        for (int i = 0; i < N; i++) {
            B[i] = A.get(i);
        }
        ...
        int maxV = B[0];
        for(int i = 1; i < N; i++) {
            int value = Integer.max(B[i] + B[i-1], B[i]);
            B[i] = value;
            maxV = Integer.max(value, maxV);
        }
